...
// --- ANAGRAFICA DI FERRO (Marzo 2026) ---
const RANK_TRUTH = {
...
function getBlindatoRank(name, apiRank) {
    if (!name) return 99;
    const n = name.toLowerCase();
    for (let key in RANK_TRUTH) {
        if (n.includes(key)) return RANK_TRUTH[key];
    }
    const r = parseInt(apiRank);
    return (r > 0 && r < 500) ? r : 78;
}
...
async function performSync(targetDate) {
    try {
        ...
        for (const match of apiMatches) {
            ...
            const rAzz = getBlindatoRank(hName, match.home_team.ranking);
            const rOpp = getBlindatoRank(aName, match.away_team.ranking);
...
app.get('/api/matchs', async (req, res) => {
    const result = await pool.query(`
        ...
        FROM matches m JOIN players p1 ON m.player_it_id = p1.id JOIN players p2 ON m.opponent_id = p2.id LEFT JOIN channels c ON m.channel_id = c.id 
        WHERE m.match_date::date = $1 ORDER BY m.match_date ASC`, [req.query.date]);
    
    res.json(result.rows.map(row => ({
        ...row,
        azz_rank_val: getBlindatoRank(row.azzurro, row.home_rank),
        opp_rank_val: getBlindatoRank(row.avversario, row.away_rank),
        temp: 23, humidity: 44, weather_icon: '☀️'
    })));
});
...
app.listen(5000, () => console.log('🛡️ MASTER V31.0: RANKING E ROI IN CASSAFORTE'));